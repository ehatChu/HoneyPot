...
<body>
	...
	<main>
		<form action="/app/board/praise" method="get">
				...
				<div class="list-bg">
					...
					<div class="page-area">
						...
						<c:if test="${pv.currentPage > 1}">
							<button type="button" onclick="location.href='/app/board/praise?p=${pv.currentPage - 1}&searchType=${searchVo.searchType}&searchValue=${searchVo.searchValue}&sortType=${searchVo.sortType}'"> < </button>
						</c:if>
						<c:forEach begin="${pv.startPage}" end="${pv.endPage}" step="1" var="i">
							<c:if test="${pv.currentPage != i}">
								<button type="button" onclick="location.href='/app/board/praise?p=${i}&searchType=${searchVo.searchType}&searchValue=${searchVo.searchValue}&sortType=${searchVo.sortType}'">${i}</button>
							</c:if>
							<c:if test="${pv.currentPage == i}">
...
<script>
	...
	firstNav(['공지 게시판', '자유 게시판', '장터 게시판', '익명 게시판', '칭찬 게시판'], '칭찬 게시판'); // 1st param : 메인 메뉴 목록, 2st param : 현재 메인 메뉴
